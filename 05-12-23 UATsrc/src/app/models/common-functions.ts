...
export const convertToMydate = (inputFormat: string | Date): string => {
  ...
  //   let parts: string[] = inputFormat.split("/");
  //   d = new Date(parts[1] + "/" + parts[0] + "/" + parts[2]); //changing date format from dd/mm/yyyy to mm/dd/yyyy
  // } else {
  // }
...
export const calculatePercent = (totalAmount: number, amount: number) => {
  let calculatedPercent = Number(((amount / totalAmount) * 100).toFixed(2));
