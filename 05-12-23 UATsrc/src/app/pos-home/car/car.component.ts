...
export class CarComponent implements OnInit, OnDestroy {
  ...
  onClickPreviousPolicyType(policy: any) {
    ...
    if (policy != "") {
      if (this.prvPolicyRememberCode == 1) {
        ...
        // this.tw.PreviousPolicyTypeCode = policy["PolicyTypeCode"];
        // this.tw.PreviousPolicyTypeDesc = policy["PolicyTypeDesc"];
        this.tw.PreviousPolicyTypeCode = policy["policyExpiredType"]["PolicyTypeCode"];
        this.tw.PreviousPolicyTypeDesc = policy["policyExpiredType"]["PolicyTypeDesc"];
        this.tw.PrvInsurerCode = policy["previousPolicyExpiredCompany"]["InsuranceCompanyCode"];
        this.tw.PrvInsurerName = policy["previousPolicyExpiredCompany"]["InsuranceCompanyDesc"];
        this.viewPolicy(this.tw);
      } else {
        this.tw.PreviousPolicyTypeCode = policy["PolicyTypeCode"];
        this.tw.PreviousPolicyTypeDesc = policy["PolicyTypeDesc"];
  ...
  onSubmitRenewCar(): any {
    ...
    this.tw.RegistrationDate = convertToMydate(frmValue.RegistrationDate);
      ...
      this._posHomeService.getVehicleInfoByNumber(frmValue.carNo).subscribe(
        (result: any): any => {
          ...
          if (result.successcode == "1") {
            ...
            tw.RegistrationDate = convertToMydate(data.RegistrationDate);
            tw.Owner = data.Owner;
  ...
  onSubmitNewCar(): any {
    ...
    tw.RegistrationDate = convertToMydate(frmValue.RegistrationDate);
