...
import { ApplicationVehicleData, errorLog, IList } from "src/app/models/common.Model";
...
@Component({
  // selector: 'app-cv-plans',
  templateUrl: './cv-plans.component.html',
  styleUrls: ['./cv-plans.component.css']
})
export class CvPlansComponent implements OnInit, OnDestroy {
  private _subscriptions: any[] = [];

  @ViewChild("cpp", { static: false })
  planCom!: CvPlanAndPremiumDetailsComponent;
  ...
  minV: number = 10000;
  maxV: number = 50000;
  unnamedPassengerCovers = ["100000", "200000"];
  // Addons params
  ...
  Legal_Liability_To_Paid_Driver: string = "0";
  Drivers = [{ code: "1", desc: "Driver 1" }, { code: "2", desc: "Driver 2" }, { code: "3", desc: "Driver 3" },
  { code: "4", desc: "Driver 4" }, { code: "5", desc: "Driver 5" }, { code: "6", desc: "Driver 6" }];
  ...
  IMT_23: number = 0;
  savedCVData!: ApplicationVehicleData;
  ...
  errors = [] as unknown as errorLog[];
  ngOnInit() {

    this.savedCVData = JSON.parse(sessionStorage.getItem("CVData")!);
    if (this.savedCVData.edit == 1) {
      this.savedCVData.edit = 0;
      sessionStorage.setItem("CVData", JSON.stringify(this.savedCVData));
    }
    ...
    // });
    this.VisiblePlan = (this.savedCVData["PreviousPolicyTypeDesc"] == 'Comrehensive') ? true : false;
  ...
  checkCurrentPolicyExDuration() {
    ...
    if (cvData.VehicleExpiryDesc == "Expired more than 90 days") {
        ...
        .subscribe((result) => {
          ...
          if (result.successcode == "1" && result.data) {
            ...
            if (result.data.Table1[0]) {
              ...
              if (bikeres.SubCateCode == 7 && bikeres.VehicleNo == "") {
                // renew
                this.yearList = [
                  { id: 1, value: "1 Year" },
                  { id: 2, value: "2 Years" },
                ...
                if (
                  ...
                  this.showTp = this.showComprehensive = true; //disable
                  // this.plan.isOdOnly = true; //check
                  this.plan.isTp = false; //uncheck
                  this.plan.isComprehensive = true;
                }
                else if (this.previousPolicyTypeCode == 7) {
                  this.showComprehensive = this.showTp = true;
                  this.plan.isTp = true; //check
                  this.plan.isComprehensive = false; //uncheck
                }
  ...
  private fetchUrlData(data: any) {
    const filterdata = {
      ...
      IDVValue: this.idvInputValue.value,
      ...
      Legal_Liability_To_Paid_Driver: this.Legal_Liability_To_Paid_Driver,
      ...
      this.vbService.getInsuranceCompanyPlan(filterdata).subscribe(
        (result) => {
          ...
          if (result.successcode == "1" && result.data != null) {
            this.showLoader = false;
            result.data.Premium = result.data.GrossAmount;
            ...
            this.gstAmount = result.data['GSTAmount'];
            ...
            if (result.data.Premium != 0) {
              ...
              result.data.MinIDVVAlue = result.data.MinIDVVAlue;
  ...
  onClickNotYourCar() {
    ...
    this._router.navigate(["/pos/cv-insurance"]);
  }
  ...
  //     // Renew Case
  //     if (
  //       +this.savedCVData.PrvPolicyExCode !== 1 &&
  //       item.CompanyCode === "9"
  //     ) {
  ...
  onBuyPlanClick1(item: IPlansMotor) {
    ...
    if (this.plan.SubCateCode == 7) {
      // Renew Case
      if (+this.savedCVData.PrvPolicyExCode !== 1) {
        // rollover case
    ...
    } else if (this.plan.SubCateCode == 8) {
      // New case
      this.onBuyPlanClick(item);
    }
  }
  ...
  onBuyPlanClick(item: any) {
    ...
    let popupData = JSON.stringify(this.popupData(item, this.postData.ApplicationNo));
    ...
    if (this.GetIPlansHealth.CompanyCode == '2'
      || this.GetIPlansHealth.CompanyCode == '6'
      || this.GetIPlansHealth.CompanyCode == '8'
      || this.GetIPlansHealth.CompanyCode == '11'
      || this.GetIPlansHealth.CompanyCode == '13'
      || this.GetIPlansHealth.CompanyCode == '16'
      || this.GetIPlansHealth.CompanyCode == '19'

    ) {
      this._router.navigate([
        `/cv-insurance/CkycDetails`,
        this.postData.ApplicationNo,
  ...
  selectTab(index: number, item: any): void {
    ...
    this.popupData(item, this.postData.ApplicationNo);
    this.planCom.PopUp = true;
    // $("#exampleModalCenter").modal("show");
  }

  popupData(item: any, ApplicationNo: number) {
    debugger;
    ...
    if (this.plan.isTp) {
      item.CoverageList = item.CoverageList.filter((x: any) => x.CoverageID != 20);
    }
    ...
    // this.premiumDataPlans["gst"] = this.gstAmount ;
    this.premiumDataPlans["gst"] = item["GSTAmount"];
    console.log(item["GSTAmount"]);
    this.premiumDataPlans["DiscountAmt"] = Number(item["DiscountAmt"]).toString().replace(/--/g, "-");
    ...
    this.premiumDataPlans["amountPayableIncludingGst"] =
      this.premiumDataPlans.Premium + +this.premiumDataPlans["gst"]
    this.premiumDataPlans["ApplicationNo"] = ApplicationNo;
  ...
  uncheckAllAddons() {
    ...
    this.Zero_Depreciation = 0,
      ...
      this.Legal_Liability_To_Paid_Driver = "0";
  ...
  removeAddonFromArray(addonArray: IAddons[], recordNo: number) {
    ...
    let addonIndex = addonArray.findIndex(
  ...
  checkAddons(event: any, item: IAddons, insuredMembersForm: any, i: any) {
    ...
    if (insuredMembersForm != undefined)
      if (item.RecordNo == 72 || item.RecordNo == 73) {
    ...
    if (event.checked) {
      ...
      if (+item.RecordNo === 72) this.ElectricalAccessories = "0";
      if (+item.RecordNo === 73) this.NonElectricalAccessories = "0";
  ...
  showSubmit() {
    ...
    for (let i = 0; i < this.checkAccessoriesValues.length; i++) {
      FoundAccessories =
        this.checkAccessoriesValues[i].RecordNo == 72 ||
        this.checkAccessoriesValues[i].RecordNo == 73;
  ...
  onSubmitAddons() {
    ...
    for (let i = 0; i < this.checkedAddons.length; i++) {
      if (this.checkedAddons[i].RecordNo == 72) {
        this.ElectricalAccessories = String(this.checkedAddons[i].value);
      }
      if (this.checkedAddons[i].RecordNo == 73) {
  ...
  showAccessories = (RecordNo: number): boolean =>
    this.checkAccessoriesValues.find(
      (f) => f.RecordNo == RecordNo && (f.RecordNo == 72 || f.RecordNo == 73)
    )
      ? false
      : true;
  ...
  private getAddonsData() {
    ...
    this._subscriptions.push(
      this.vbService.getAddonsListForCV().subscribe((result) => {
        if (result.successcode == "0") {
          this.errorLogDetails.UserCode = this.posMobileNo;
          this.errorLogDetails.ApplicationNo = this.postData.ApplicationNo;
          this.errorLogDetails.CompanyName = ''
          this.errorLogDetails.ControllerName = 'PosHome'
          this.errorLogDetails.MethodName = 'GetAddonsForCommercialVehicles'
          this.errorLogDetails.ErrorCode = result.successcode
          this.errorLogDetails.ErrorDesc = result.msg;
          this._errorHandleService
            .sendErrorLog(this.errorLogDetails)
  ...
  // onChooseUnnamedCover(item: IAddons, cover) {
  //   console.log(cover);
  //   
  ...
  onChooseDriverCover(item: IAddons, cover: any) {
    console.log("covae4r gfds", item, cover);
    ...
    if (this.selectedUnnamedCover != cover) {
      ...
      this.selectedUnnamedCover = cover;
      this.Legal_Liability_To_Paid_Driver = this.selectedUnnamedCover.code;
      let addonPresentIndex = this.checkedAddons.findIndex(
        (addon) => +addon.RecordNo === +item.RecordNo
      );
  ...
  otherEffectsOnAddonClick(event: any, item: IAddons) {
    if (event.checked) {
      ...
      // }
      if (+item.RecordNo === 76) this.plan.paCover = true;
      // if (+item.RecordNo === 17) {
      //   this.removeAddonFromArray(this.checkedAddons, +item.RecordNo);
      ...
      if (+item.RecordNo === 80) {
        this.removeAddonFromArray(this.checkedAddons, +item.RecordNo);
        this.showDriversCover = true;
      }
    } else {
      // $("#AddonsClose").click();
      this.AddCoverDisplay = "none";
      this.AddonsClose = false;
      if (+item.RecordNo === 61) this.plan.zeroDp = false;
      // if (
      //   +item.RecordNo === 2 ||
      //   +item.RecordNo === 4 ||
      //   +item.RecordNo === 6 ||
      ...
      // }
      if (+item.RecordNo === 76) this.plan.paCover = false;
      // if (+item.RecordNo === 17) {
  ...
  addons(event: any, item: IAddons) {
    ...
    if (event.checked) {
      ...
      if (item.RecordNo == 75) this.Paid_Driver_Cover = 1;
      if (item.RecordNo == 76) {
      ...
      if (item.RecordNo == 75) this.Paid_Driver_Cover = 0;
      if (item.RecordNo == 76) {
        this.Owner_Driver_PA_Cover = 0;
        // this.plan.paCover = false;
      }
  ...
  callForQuotation() {
    // $("#AddonsClose").click();
    // this.AddonsClose = true;
    // $("#showAntiTheftSubmitBtn").trigger("click")
    $("#submitAddonBtn").trigger('click');

    this.additionalCoverDisable = true;
    this.AddCoverDisplay = "none";
    this.AddonsClose = false;
    let zeroDpExists = this.checkedAddons.findIndex(x => x.RecordNo == 61)
    let paCoverExists = this.checkedAddons.findIndex(x => x.RecordNo == 76)
  ...
  premiumWithoutDiscount(item: any) {
    ...
    if (ncb && +DiscountAmt) {
      if (!this.plan.isTp)
        actualAmount =
          +ncb.ODPremium != 0
            ? String(+item.Premium - +ncb.ODPremium + DiscountAmt)
            : "";
    } else if (ncb) {
      if (!this.plan.isTp)
        actualAmount =
          +ncb.ODPremium != 0
            ? String(+item.Premium - +ncb.ODPremium)
            : "";
  ...
  async captureScreen() {
    ...
    try {
      ...
      capturePromise.subscribe((imgString: any) => {
        ...
        let policyType = this.savedCVData["PreviousPolicyTypeDesc"]
          ? this.savedCVData["PreviousPolicyTypeDesc"]
          : this.savedCVData["SubCateDesc"];
