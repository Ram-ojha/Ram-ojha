...
import { EBajajDocListEnum, EDocListEnum, EYesNo } from "src/app/models/insurance.enum";
import {
  IState,
  ICity,
  IInsuranceCompany,
...
@Component({
  selector: 'app-buyplan-go-digit',
  templateUrl: './buyplan-go-digit.component.html',
  styleUrls: ['./buyplan-go-digit.component.css', "../../buyplan/buyplan.component.css", "../../../bike-insurance/bike-plans/bike-plans.component.css"]
})
export class BuyplanGoDigitComponent implements OnInit, OnDestroy {
  ...
  identityMask!: { guide: boolean; keepCharPositions: boolean; showMask: boolean; mask: RegExp[]; };
  ckycDetailForm: FormGroup;
  goDigitCkycDocList!: ICKYCDocList;
  isCompleted_ckycInfo!: boolean;
  fileName: any='';
  base64url!: string;
  ...
  getCkycDocList() {
    this._subscriptions.push(
      this.posHomeService.getCKYCDocList(this.PlanData.CompanyCode).subscribe((result: any) => {
        this.goDigitCkycDocList = result.data;
      })
    )
  }
...
// console.log(city);
  ...
  //     doc.setValidators([
  //       (): ValidationErrors => { return { fileExtensionError: "Please Upload Only jpg, jpeg, bmp ..." } }
  //     ]);
  //     doc.updateValueAndValidity();
  //     console.log(doc)
  //     return;
  ...
  //     doc.setValidators([
  //       (): ValidationErrors => { return { fileSizeError: "Max file size allowed is 4 MB. Allowed file type Only  jpg, bmp." } }
  //     ]);
  //     doc.updateValueAndValidity();
  //     console.log(doc)
