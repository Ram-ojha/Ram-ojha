...
@Component({
  templateUrl: "./buyplantataaig.component.html",
  styleUrls: ["./buyplantataaig.component.css"],
  encapsulation: ViewEncapsulation.None,
})
export class BuyplanTataaigComponent implements OnInit, OnDestroy {
  ...
  private updateIdentityValidity() {
    ...
    this._subscriptions.push(
      this.communicationAddressForm
        .get("identity")!
        .valueChanges.subscribe((identity: IYesNoInd | undefined | null) => {
          control!.reset();
  ...
  appointeedetails(date: any) {
    ...
    if (yearCount < 18) {
      ...
      this.nomineeForm.controls["AppointeeAddress"].setValidators([
        Validators.required,
      ]);
    } else {
      ...
      this.nomineeForm.controls["AppointeeAddress"].setValue("");
      ...
      this.nomineeForm.controls["AppointeeAddress"].setValidators(null);
    ...
    this.nomineeForm.controls["AppointeeAddress"].updateValueAndValidity();
  }
  ...
  private createQAForm = async (tree: IQuestionsTree[]) => {
    ...
    // code by Jayam
    for (let row of this.companyQueTree) {
      for (let item of row.Childrens) {
        for (let i = 0; i < item.Members.length; i++) {
          let queId = this.qnaForm.controls[item.QuestionID] as FormArray
          for (let i = 0; i < queId['controls'].length; i++) {
            if (item.QuestionID == 218) {
              // • Insured having Smoking Intake = ‘Weekly’ and Smoking limit > 70 cigarettes
              queId['controls'][i].setValidators([
                Validators.max(70)
              ]);
              queId['controls'][i].updateValueAndValidity();
            }
            if (item.QuestionID == 222) {
              // • Insured having Alcohol Intake = ‘Weekly’ and Alcohol Quantity > 1200 ml
              queId['controls'][i].setValidators([
                Validators.max(40)
              ]);
              queId['controls'][i].updateValueAndValidity();
            }
            if (item.QuestionID == 220) {
              // • 245 gm
              queId['controls'][i].setValidators([
                Validators.max(49)
              ]);
  ...
  onSubmitMemberDetails(insuredMembersForm: NgForm): any {
    ...
    for (let i = 0; i < this.memberDetails.length; i++) {
      ...
      if (this.memberDetails[i].Age < 18) {
        ...
        this.OccupationCode = this.memberDetails[
          i
        ].OccupationDesc.OccupationCode;
        this.OccupationDesc = this.memberDetails[
          i
              ...
              if (this.companyQueTree[i].LoadTypeInd == 1) {
                ...
                this.medicalHis.find(
                  (item) =>
                    item.RecordNo == this.medicalHisAns[S].RecordNo &&
                    item.QuestionID == this.medicalHisAns[S].QuestionID
                ).MedicalLoadingPercentage = this.companyQueTree[
                  i
                ].CompanyLoadValue;
              }
    ...
    this._subscriptions.push(
      this._healthBuyPlanService
        ...
        .subscribe(
          (result: ApiResponse) => {
            this.showLoader = false;
            let proposerForm: number | undefined;
            let insuredMembers: number;
            let addressindex: number;
            let nomineeForm: number;
            let medicalHis: number;
