import { Component, OnDestroy, OnInit } from "@angular/core";
import { HealthBuyPlanService } from "../../services/health-buyplan.service";
import { decrypt } from "src/app/models/common-functions";
import { ActivatedRoute } from "@angular/router";
import jsPDF from "jspdf";
import { Covers } from "src/app/models/common";
import { ErrorHandleService } from "src/app/shared/services/error-handler.service";
import { errorLog } from "src/app/models/common.Model";
import { ToastrService } from "ngx-toastr";

@Component({
  selector: "health-payment",
  templateUrl: "./health-payment.component.html",
  styleUrls: ["./health-payment.component.css"],
})
export class HealthPaymentComponent implements OnInit, OnDestroy {
  private _subscriptions: any[] = [];
  showLoader = false;
  policyData: any;
  additionaldata: any;
  comp_code!: string;
  comp_data!: string;
  errorMessage!: string;
  download: boolean = true;
  successStatus: string = "0";
  coverList = Covers;
  SumInsured!: string;
  regNo!: string;

  constructor(
    private _healthBuyPlanService: HealthBuyPlanService,
    private _route: ActivatedRoute,
    public _errorHandleService: ErrorHandleService,
    private _toastService: ToastrService
  ) { }

  posMobileNo: any;
  errorLogDetails = {} as unknown as errorLog;
  ApplicationNo: any;
  ngOnInit() {

    this.ApplicationNo = decrypt(sessionStorage.getItem("appNo")!);
    this.posMobileNo = decrypt(sessionStorage.getItem("posMob")!);
    this._subscriptions.push(
      this._route.paramMap.subscribe((p: any) => {
        this.comp_code = p.get("comp_code");
        this.comp_data = p.get("comp_data");
        console.log("comp_code", this.comp_code, "comp_data", this.comp_data);
      })
    );
    this.errorMessage = "Error";
    this.download = false;

    var proposalNo = decrypt(sessionStorage.getItem("proposalNo")!);
    var appNo = decrypt(sessionStorage.getItem("appNo")!);
    this.regNo = decrypt(sessionStorage.getItem("regNo")!);

    console.log("proposalNo2", proposalNo);
    console.log("appNo2", appNo);
    console.log("regNo2", this.regNo);

    // nirmal code add Start
    var PartnerId = decrypt(sessionStorage.getItem("PId")!);
    var Com = (sessionStorage.getItem("companyData"));
    console.log(PartnerId);
    // var PartnerIdforhdfc =(sessionStorage.getItem("p_id"));
    var ProductCode = decrypt(sessionStorage.getItem("PCode")!);
    // nirmal code add End

    // this.showLoader = true;
    let url = this.comp_data.replace(/-/g, "/");
    if (this.comp_code == "1") {
      this.GetReligarePolicyPdf(url, proposalNo, appNo, this.regNo);
    } else if (this.comp_code == "2") {
      const data = {
        PartnerId: PartnerId,
        ProductCode: ProductCode,
        ApplicationNo: appNo,
        ApplicationNoOdp: 0,
        RegistrationNo: this.regNo,
        RegistrationNoOdp: 0,
      };
      this.GetFuturePolicyPdf(url, data);
    } else if (this.comp_code == "3") {
      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        PurchaseToken: this.comp_data.replace(/-/g, "/"),
      };
      console.log("Url Data ==> ", data);
      this.GetStarPolicyPdf(data);
    } else if (this.comp_code == "6") {
      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        PurchaseToken: this.comp_data.replace(/-/g, "/"),
      };
      console.log("Url Data ==> ", data);
      this.GetUniversalSompoHealth(url, data);
    } else if (this.comp_code == "8") {
      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        PurchaseToken: this.comp_data.replace(/-/g, "/"),
      };
      console.log("Url Data ==> ", data);
      this.GetGoDigitHealth(url, data);
    } else if (this.comp_code == "9") {

      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        PurchaseToken: this.comp_data.replace(/-/g, "/"),
      };
      console.log("Url Data ==> ", data);
      this.GetTataAIGHealth(url, data);
    } else if (this.comp_code == "10") {

      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        PurchaseToken: this.comp_data.replace(/-/g, "/"),
      };
      console.log("Url Data ==> ", data);
      this.GetGetNationalInsuranceHealth(url, data);
    } else if (
      this.comp_code == "11" ||
      this.comp_code == "7" ||
      // this.comp_code == "14" ||
      this.comp_code == "12"
    ) {
      // Reliance

      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        ProposalNumber: proposalNo,
        // CompanyCode: this.comp_code,

        ProductCode: ProductCode,
      };
      console.log("Url Data ==> ", data);
      this.getReliancePdf(url, data);
    }
    else if (this.comp_code == "14") {
      //hdfc
      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        ProposalNumber: proposalNo,
        // CompanyCode: this.comp_code,
        PartnerId: PartnerId,
        ProductCode: ProductCode,
      };
      console.log("Url Data ==> ", data);
      this.getReliancePdf(url, data);
    }

    else if (this.comp_code == "4") {

      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        ProposalNumber: proposalNo,
        // CompanyCode: this.comp_code,
        // ProductCode: ProductCode,
      };
      this.getReliancePdf(url, data); // reliance too doing the same thing, so reusing it
    } else if (this.comp_code == "7") {
      // SBI

      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        ProposalNumber: proposalNo,
        // CompanyCode: this.comp_code,
        // ProductCode: ProductCode,
      };
      console.log("Url Data ==> ", data);
      this.getReliancePdf(url, data); // reliance too doing the same thing, so reusing it
    }
    else if (this.comp_code == "17") {
      // NivaBupa
      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        ProposalNumber: proposalNo,
        // CompanyCode: this.comp_code,
        // ProductCode: ProductCode,
      };
      // this.getNivaBupaPdf(url, data)
      this.GetAfterPaymentHealthPolicyStatus(url)
    }
  }

  GetReligarePolicyPdf(url: string, proposalNo: string, appNo: string, regNo: string) {
    this.showLoader = true;
    this._subscriptions.push(
      this._healthBuyPlanService
        .GetReligarePolicyPdf(url, proposalNo, appNo, regNo)
        .subscribe((res) => {
          console.log("Api Response", res);
          this.showLoader = false;
          this.successStatus = res.successcode;
          this.policyData = res.data;

          if (res.successcode == "0" || res.successcode == null) {
            this.errorLogDetails.UserCode = this.posMobileNo;
            this.errorLogDetails.ApplicationNo = this.ApplicationNo;
            this.errorLogDetails.CompanyName = url.split("/")[0];
            this.errorLogDetails.ControllerName = url.split("/")[0];
            this.errorLogDetails.MethodName = url.split("/")[1];
            this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
            this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
            this._errorHandleService
              .sendErrorLog(this.errorLogDetails)
              .subscribe((res: any) => {
                console.log(res);
              });
          }
          if (res.successcode == "1") {
            let a = this.coverList.filter(
              (m) => m.id == this.policyData.SumInsured
            );
            this.SumInsured = a[0].value;
          }
        })
    );
  }
  GetStarPolicyPdf(data: any) {
    this._subscriptions.push(
      this._healthBuyPlanService.GetStarPolicyPdf(data).subscribe((res) => {
        console.log("Api Response", res);
        this.showLoader = false;
        this.successStatus = res.successcode;
        this.policyData = res.data[0];

        if (res.successcode == "0" || res.successcode == null) {
          this.errorLogDetails.UserCode = this.posMobileNo;
          this.errorLogDetails.ApplicationNo = this.ApplicationNo;
          this.errorLogDetails.CompanyName = '';
          this.errorLogDetails.ControllerName = `Star`;
          this.errorLogDetails.MethodName = `CheckPolicyStatus`;
          this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
          this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
          this._errorHandleService
            .sendErrorLog(this.errorLogDetails)
            .subscribe((res: any) => {
              console.log(res);
            });
        }
        if (res.successcode == "1") {
          let a = this.coverList.filter(
            (m) => m.id == this.policyData.SumInsured
          );
          this.SumInsured = a[0].value;
        }
      })
    );
  }

  GetFuturePolicyPdf(url: any, data: any) {
    this.showLoader = true;
    console.log(url);
    this._subscriptions.push(
      this._healthBuyPlanService
        .GetFuturePolicyPdf(url, data)
        .subscribe((res) => {
          this.showLoader = false;
          this.successStatus = res.successcode;
          this.policyData = res.data;
          this.additionaldata = res.additionaldata;

          if (res.successcode == "0" || res.successcode == null) {
            this.errorLogDetails.UserCode = this.posMobileNo;
            this.errorLogDetails.ApplicationNo = this.ApplicationNo;
            this.errorLogDetails.CompanyName = url.split['/'][0];
            this.errorLogDetails.ControllerName = url.split['/'][0];
            this.errorLogDetails.MethodName = url.split['/'][1];
            this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
            this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
            this._errorHandleService
              .sendErrorLog(this.errorLogDetails)
              .subscribe((res: any) => {
                console.log(res);
              });
          }

          if (res.successcode == "1") {
            let a = this.coverList.filter(
              (m) => m.id == this.policyData.SumInsured
            );
            this.SumInsured = a[0].value;
          }
          console.log(this.policyData);
        })
    );
  }

  GetUniversalSompoHealth(url: any, data: any) {
    this.showLoader = true;
    this._subscriptions.push(
      this._healthBuyPlanService
        .GetUniversalSompoHealth(url, data)
        .subscribe((res) => {
          this.showLoader = false;
          this.successStatus = res.successcode;
          this.policyData = res.data;
          if (res.successcode == "0" || res.successcode == null) {
            this.errorLogDetails.UserCode = this.posMobileNo;
            this.errorLogDetails.ApplicationNo = this.ApplicationNo;
            this.errorLogDetails.CompanyName = url.split['/'][0];
            this.errorLogDetails.ControllerName = url.split['/'][0];
            this.errorLogDetails.MethodName = url.split['/'][1];
            this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
            this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
            this._errorHandleService
              .sendErrorLog(this.errorLogDetails)
              .subscribe((res: any) => {
                console.log(res);
              });
          }
          if (res.successcode == "1") {
            let a = this.coverList.filter(
              (m) => m.id == this.policyData.SumInsured
            );
            this.SumInsured = a[0].value;
          }
          console.log(this.policyData);
        })
    );
  }

  GetGoDigitHealth(url: any, data: any) {
    this.showLoader = true;
    this._subscriptions.push(
      this._healthBuyPlanService
        .GetGoDigitHealth(url, data)
        .subscribe((res) => {
          this.showLoader = false;
          this.successStatus = res.successcode;
          this.policyData = res.data[0];
          this.additionaldata = res.additionaldata;
          this.additionaldata != null
            ? (this.additionaldata = JSON.parse(res.additionaldata))
            : (this.additionaldata = "");
          console.log("--------------------", this.additionaldata);
          if (res.successcode == "0" || res.successcode == null) {
            this.errorLogDetails.UserCode = this.posMobileNo;
            this.errorLogDetails.ApplicationNo = this.ApplicationNo;
            this.errorLogDetails.CompanyName = url.split['/'][0];
            this.errorLogDetails.ControllerName = url.split['/'][0];
            this.errorLogDetails.MethodName = url.split['/'][1];
            this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
            this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
            this._errorHandleService
              .sendErrorLog(this.errorLogDetails)
              .subscribe((res: any) => {
                console.log(res);
              });
          }
          if (res.successcode == "1") {
            let a = this.coverList.filter(
              (m) => m.id == this.policyData.SumInsured
            );
            this.SumInsured = a[0].value;
          }
          console.log(this.policyData);
        })
    );
  }

  GetTataAIGHealth(url: any, data: any) {
    this.showLoader = true;
    this._subscriptions.push(
      this._healthBuyPlanService
        .GetTataAIGHealth(url, data)
        .subscribe((res) => {
          console.log("----------------", res);
          this.showLoader = false;
          this.successStatus = res.successcode;
          this.policyData = res.data[0];

          if (res.successcode == "0" || res.successcode == null) {
            this.errorLogDetails.UserCode = this.posMobileNo;
            this.errorLogDetails.ApplicationNo = this.ApplicationNo;
            this.errorLogDetails.CompanyName = url.split['/'][0];
            this.errorLogDetails.ControllerName = url.split['/'][0];
            this.errorLogDetails.MethodName = url.split['/'][1];
            this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
            this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
            this._errorHandleService
              .sendErrorLog(this.errorLogDetails)
              .subscribe((res: any) => {
                console.log(res);
              });
          }

          if (res.successcode == "1") {
            let a = this.coverList.filter(
              (m) => m.id == this.policyData.SumInsured
            );
            this.SumInsured = a[0].value;
          }
          console.log(this.policyData);
        })
    );
  }

  GetGetNationalInsuranceHealth(url: any, data: any) {
    this.showLoader = true;
    this._subscriptions.push(
      this._healthBuyPlanService
        .GetNationalInsuranceHealth(url, data)
        .subscribe((res) => {
          console.log("----------------", res);
          this.showLoader = false;
          this.successStatus = res.successcode;
          this.policyData = res.data;
          if (res.successcode == "0" || res.successcode == null) {
            this.errorLogDetails.UserCode = this.posMobileNo;
            this.errorLogDetails.ApplicationNo = this.ApplicationNo;
            this.errorLogDetails.CompanyName = url.split['/'][0];
            this.errorLogDetails.ControllerName = url.split['/'][0];
            this.errorLogDetails.MethodName = url.split['/'][1];
            this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
            this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
            this._errorHandleService
              .sendErrorLog(this.errorLogDetails)
              .subscribe((res: any) => {
                console.log(res);
              });
          }
          if (res.successcode == "1") {
            let a = this.coverList.filter(
              (m) => m.id == this.policyData.SumInsured
            );
            this.SumInsured = a[0].value;
          }
          console.log(this.policyData);
        })
    );
  }

  getReliancePdf(url: any, data: any) {

    this.showLoader = true;
    this._subscriptions.push(
      this._healthBuyPlanService.getReliance(url, data).subscribe((res) => {
        console.log("----------------", res);
        this.showLoader = false;
        this.successStatus = res.successcode;
        this.policyData = res.data;
        if (res.successcode == "0" || res.successcode == null) {
          this.errorLogDetails.UserCode = this.posMobileNo;
          this.errorLogDetails.ApplicationNo = this.ApplicationNo;
          this.errorLogDetails.CompanyName = url.split['/'][0];
          this.errorLogDetails.ControllerName = url.split['/'][0];
          this.errorLogDetails.MethodName = url.split['/'][1];
          this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
          this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
          this._errorHandleService
            .sendErrorLog(this.errorLogDetails)
            .subscribe((res: any) => {
              console.log(res);
            });
        }
        if (res.successcode == "1") {
          let a = this.coverList.filter(
            (m) => m.id == this.policyData.SumInsured
          );
          this.SumInsured = a[0].value;
        }
        console.log(this.policyData);
      })
    );
  }
  GetAfterPaymentHealthPolicyStatus(url: any) {
    console.log();
    this._subscriptions.push(
      this._healthBuyPlanService.GetAfterPaymentHealthPolicyStatus(this.regNo).subscribe((res: any) => {
        console.log("----------------", res);
        this.showLoader = false;
        this.successStatus = res.successcode;
        this.policyData = res.data;
        if (res.successcode == "0" || res.successcode == null) {
          this.errorLogDetails.UserCode = this.posMobileNo;
          this.errorLogDetails.ApplicationNo = this.ApplicationNo;
          this.errorLogDetails.CompanyName = url.split('/')[0];
          this.errorLogDetails.ControllerName = url.split('/')[0];
          this.errorLogDetails.MethodName = url.split('/')[1];
          this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
          this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
          this._errorHandleService
            .sendErrorLog(this.errorLogDetails)
            .subscribe((res: any) => {
              console.log(res);
            });
        }
        // if (res.successcode == "1") {
        //   let a = this.coverList.filter(
        //     (m) => m.id == this.policyData.SumInsured
        //   );
        //   this.SumInsured = a[0].value;
        // }
        console.log(this.policyData[0].Name);
      })
    )
  }

  getNivaBupaPdf(url: any, data: any) {

    let Url = url + `?RegistrationNo=${data.RegistrationNo}`
    // let Url = url + '?' + 'RegistrationNo' + '=' + data.RegistrationNo
    console.log(Url, url);
    this.showLoader = true;
    this._subscriptions.push(
      this._healthBuyPlanService.getNivaBupa(Url, data).subscribe((res) => {
        console.log("----------------", res);

        if (res.successcode == "0" || res.successcode == null) {
          this._toastService.error(res.msg, 'PDF Download Failed!');
          this.errorLogDetails.UserCode = this.posMobileNo;
          this.errorLogDetails.ApplicationNo = this.ApplicationNo;
          this.errorLogDetails.CompanyName = url.split('/')[0];
          this.errorLogDetails.ControllerName = url.split('/')[0];
          this.errorLogDetails.MethodName = url.split('/')[1];
          this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
          this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
          this._errorHandleService
            .sendErrorLog(this.errorLogDetails)
            .subscribe((res: any) => {
              console.log(res);
            });
        }
        if (res.successcode == "1") {
          let a = this.coverList.filter(
            (m) => m.id == this.policyData.SumInsured
          );
          this.SumInsured = a[0].value;
        }
        console.log(this.policyData);
      })
    );
  }
  getIffcoTokio(url: any, data: any) {

    this.showLoader = true;
    this._subscriptions.push(
      this._healthBuyPlanService.getIffcoTokio(url, data).subscribe((res) => {
        console.log("----------------", res);
        this.showLoader = false;
        this.successStatus = res.successcode;
        this.policyData = res.data;
        if (res.successcode == "0" || res.successcode == null) {
          this.errorLogDetails.UserCode = this.posMobileNo;
          this.errorLogDetails.ApplicationNo = this.ApplicationNo;
          this.errorLogDetails.CompanyName = url.split['/'][0];
          this.errorLogDetails.ControllerName = url.split['/'][0];
          this.errorLogDetails.MethodName = url.split['/'][1];
          this.errorLogDetails.ErrorCode = res.successcode ? res.successcode : "0";
          this.errorLogDetails.ErrorDesc = res.msg ? res.msg : "Something went wrong.";
          this._errorHandleService
            .sendErrorLog(this.errorLogDetails)
            .subscribe((res: any) => {
              console.log(res);
            });
        }
        if (res.successcode == "1") {
          let a = this.coverList.filter(
            (m) => m.id == this.policyData.SumInsured
          );
          this.SumInsured = a[0].value;
        }
        console.log(this.policyData);
      })
    );
  }

  Repayment() {
    if (this.comp_code === "1") {
      let param = {
        ApplicationNo: this.policyData.ApplicationNo,
        proposalNum: this.policyData.ProposalNumber,
        ProposalPremium: this.policyData.ProposalPremium,
        RegistrationNo: this.policyData.RegistrationNo,
        returnURL: this.policyData.ResponseURL,
      };
      this.postFormReligare(this.policyData.PaymentURL, param, "");
    } else if (this.comp_code === "2") {
      var OtherParams = this.policyData.OtherParams.split("|");
      let param = {
        TransactionID: this.policyData.RegistrationNo,
        PaymentOption: OtherParams[0],
        ResponseURL: this.policyData.ResponseURL,
        ProposalNumber: decrypt(sessionStorage.getItem("proposalNo")!),
        PremiumAmount: this.policyData.ProposalPremium,
        UserIdentifier: OtherParams[6],
        UserId: OtherParams[1],
        FirstName: OtherParams[2],
        LastName: OtherParams[3],
        Mobile: OtherParams[4],
        Email: OtherParams[5],
        CheckSum: this.additionaldata,
      };
      this.postFormFuture(this.policyData.PaymentURL, param, "");
    } else if (this.comp_code === "3") {
      var url = decrypt(sessionStorage.getItem("url")!);
      window.location.href = url;
    } else if (this.comp_code == "6") {
      window.location.href = this.policyData.PaymentURL;
    } else if (this.comp_code == "8" || this.comp_code == "4") {
      var url = decrypt(sessionStorage.getItem("url")!);
      window.location.href = url;
    } else if (this.comp_code == "9") {
      let param = {
        proposal_no: this.policyData.ProposalNumber,
        src: "TP",
      };
      this.postFormTataAIG(this.policyData.PaymentURL, param, "");
    } else if (this.comp_code == "10") {
      window.location.href = this.policyData.PaymentURL;
    }
  }

  private postFormReligare(
    path: string,
    params: { [x: string]: string; proposalNum: string; returnURL: string },
    method: string
  ) {
    method = method || "post";

    var form = document.createElement("form");
    form.setAttribute("method", method);
    form.setAttribute("action", path);
    form.setAttribute("name", "PAYMENTFORM");

    for (var key in params) {
      if (params.hasOwnProperty(key)) {
        var hiddenField = document.createElement("input");
        hiddenField.setAttribute("type", "hidden");
        hiddenField.setAttribute("name", key);
        hiddenField.setAttribute("value", params[key]);

        form.appendChild(hiddenField);
      }
    }

    document.body.appendChild(form);
    form.submit();
  }

  private postFormFuture(
    path: string,
    params: any,
    method: string
  ) {
    method = method || "post";

    var form = document.createElement("form");
    form.setAttribute("method", method);
    form.setAttribute("action", path);
    form.setAttribute("name", "form1");
    form.setAttribute("id", "form1");
    form.setAttribute("style", "display:none");

    for (var key in params) {
      if (params.hasOwnProperty(key)) {
        var hiddenField = document.createElement("input");
        hiddenField.setAttribute("type", "text");
        hiddenField.setAttribute("name", key);
        hiddenField.setAttribute("value", params[key]);

        form.appendChild(hiddenField);
      }
    }

    document.body.appendChild(form);
    form.submit();
  }

  private postFormTataAIG(
    path: string,
    params: any,
    method: string
  ) {
    method = method || "post";

    var form = document.createElement("form");
    form.setAttribute("method", method);
    form.setAttribute("action", path);
    form.setAttribute("name", "PAYMENTFORM");

    for (var key in params) {
      if (params.hasOwnProperty(key)) {
        var hiddenField = document.createElement("input");
        hiddenField.setAttribute("type", "hidden");
        hiddenField.setAttribute("name", key);
        hiddenField.setAttribute("value", params[key]);

        form.appendChild(hiddenField);
      }
    }

    document.body.appendChild(form);
    form.submit();
  }

  PolicydownloadPdf() {
    if (this.comp_code == "1") {

      this.showLoader = true;
      const linkSource = this.policyData.PolicyDoc;
      const downloadLink = document.createElement("a");
      const fileName = "PolicyNo_" + this.policyData.PolicyNumber + ".pdf";

      downloadLink.href = linkSource;
      downloadLink.download = fileName;
      downloadLink.click();
      this.showLoader = false;
    } else if (this.comp_code == "2") {

      this.showLoader = true;
      const linkSource = this.policyData.PolicyDoc;
      const downloadLink = document.createElement("a");
      const fileName = "PolicyNo_" + this.policyData.PolicyNumber + ".pdf";

      downloadLink.href = linkSource;
      downloadLink.download = fileName;
      downloadLink.click();
      this.showLoader = false;
    } else if (this.comp_code == "8") {
      this.showLoader = true;
      if (this.additionaldata != "") {
        const linkSource = this.additionaldata.proposalPath;
        const downloadLink = document.createElement("a");
        const fileName = this.policyData.PolicyNumber + ".pdf";

        downloadLink.href = linkSource;
        downloadLink.download = fileName;
        downloadLink.click();
      } else {
        this._errorHandleService._toastService.info("Please try after sometime");
      }
      this.showLoader = false;
    } else if (this.comp_code == "9") {
      const linkSource =
        "https://pipuat.tataaiginsurance.in/tagichubws/medicare.jsp?polno=" +
        this.policyData.PolicyNumber +
        "&src=app&key=" +
        this.policyData.TransactionNo +
        "";
      const downloadLink = document.createElement("a");
      const fileName = "ddd.pdf";

      downloadLink.href = linkSource;
      downloadLink.download = fileName;
      downloadLink.click();
    } else if (this.comp_code == "10") {
      this.showLoader = true;
      const linkSource = this.policyData.PolicyDoc;
      const downloadLink = document.createElement("a");
      const fileName = "PolicyNo_" + this.policyData.PolicyNumber + ".pdf";

      downloadLink.href = linkSource;
      downloadLink.download = fileName;
      downloadLink.click();
      this.showLoader = false;
    } else if (
      this.comp_code == "11" ||
      this.comp_code == "7"
      // ||
      // this.comp_code == "14"
    ) {

      this.showLoader = true;
      const linkSource = this.policyData.ResponseURL;
      const downloadLink = document.createElement("a");
      const fileName = "PolicyNo_" + this.policyData.PolicyNumber + ".pdf";

      if (!linkSource) return;
      downloadLink.href = linkSource;
      downloadLink.download = fileName;
      downloadLink.click();
      this.showLoader = false;
    } else if (this.comp_code == "14") {
      // hdfc ergo
      // provides base 64

      const linkSource =
        "data:application/pdf;base64," + this.policyData.PolicyDoc;
      const downloadLink = document.createElement("a");
      const fileName = "PolicyNo_" + this.policyData.PolicyNumber + ".pdf";

      if (!linkSource) return;
      downloadLink.href = linkSource;
      downloadLink.download = fileName;
      downloadLink.click();
    } else if (this.comp_code == "4") {

      this.showLoader = true;
      const linkSource = this.policyData.ResponseURL;
      const downloadLink = document.createElement("a");
      const fileName = "PolicyNo_" + this.policyData.PolicyNumber + ".pdf";

      if (!linkSource) return;
      downloadLink.href = linkSource;
      downloadLink.download = fileName;
      downloadLink.click();
      this.showLoader = false;
    } else if (this.comp_code == "12") {
      this.showLoader = true;
      const linkSource = this.policyData.PolicyDoc;
      const downloadLink = document.createElement("a");
      const fileName = "PolicyNo_" + this.policyData.PolicyNumber + ".pdf";

      downloadLink.href = linkSource;
      downloadLink.download = fileName;
      downloadLink.click();
      this.showLoader = false;
    } else if (this.comp_code == "17") {
      let url = this.comp_data.replace(/-/g, "/")
      var proposalNo = decrypt(sessionStorage.getItem("proposalNo")!);
      var appNo = decrypt(sessionStorage.getItem("appNo")!);
      const data = {
        RegistrationNo: this.regNo,
        ApplicationNo: appNo,
        ProposalNumber: proposalNo,
        // CompanyCode: this.comp_code,
        // ProductCode: ProductCode,
      };
      this.getNivaBupaPdf(url, data)
    }
    else {
      this.showLoader = true;
      const linkSource = this.policyData.PolicyDoc;
      const downloadLink = document.createElement("a");
      const fileName = "PolicyNo_" + this.policyData.PolicyNumber + ".pdf";
      downloadLink.href = linkSource;
      downloadLink.download = fileName;
      downloadLink.click();
      this.showLoader = false;
    }
  }
  ngOnDestroy() {
    this._subscriptions.forEach((sub: any) => sub.unsubscribe());
  }
  // showPdf() {
  //   const linkSource = 'data:application/pdf;base64,' + '';
  //   const downloadLink = document.createElement("a");
  //   const fileName = this.policyData.PolicyNumber + ".pdf";

  //   downloadLink.href = linkSource;
  //   downloadLink.download = fileName;
  //   downloadLink.click();
  // }
}
