...
@Component({
  selector: 'app-buyplan-iffcotokio',
  templateUrl: './buyplan-iffcotokio.component.html',
  styleUrls: ['./buyplan-iffcotokio.component.css', "../../buyplan/buyplan.component.css","../../../bike-insurance/bike-plans/bike-plans.component.css"]
})
export class BuyplanIffcotokioComponent implements OnInit,OnDestroy {
  ...
  onEnterRegNoForNew(){
    const v: any =this.vehicleDetailForm.get("regNo");
    console.log(v)
    const vSplit=v.value.split("-")
    if(v.value.length>0 ){
      if (!vSplit[1]) {//|| vSplit[1].length < 4
          v.setErrors({})
      } else if(vSplit[1] && vSplit[1].length < 4){
        v.setErrors({})
      }else{
        v.setErrors(null)
      }
      ...
      this._vehicleBuyPlanService.savePraposal(this.postData).subscribe(
        (result) => {
          if (result.successcode == "1") {
            ...
            // this._router.navigate([`/bike-insurance/CkycDetails/`,appNo,appOdpNo])//(`/bike-insurance/buyplan/${appNo}/${appOdpNo}`);//this._location.replaceState(`/bike-insurance/buyplan/`,appNo,appOdpNo);//
            // console.log("url  ",url);
...
export const pastpolicyExDateValidationForTATA = (
  ...
  return (control: AbstractControl): ValidationErrors |null => {
    ...
    if (monthDiff <= 9){ return { postPolicyDateError: true };}else{return null}
  };
};
