...
@Component({
  selector: 'app-buyplan-sbi',
  templateUrl: './buyplan-sbi.component.html',
  styleUrls: ['./buyplan-sbi.component.css', "../../buyplan/buyplan.component.css", "../../../bike-insurance/bike-plans/bike-plans.component.css"]
})
export class BuyplanSbiComponent implements OnInit, OnDestroy {
  ...
  public genderList: IGender[] = [
    { GenderCode: 1, GenderDesc: "Male" },
    { GenderCode: 2, GenderDesc: "Female" },
    { GenderCode: 3, GenderDesc: "Other" }
  ];
  public familyMemberList: IFamilyMembers[] = [];
  public stateList: IState[] = [];
  public cityList: ICitySBI[] = [];
  public districtList: IDistrict[] = [];
  public pincodeList: IPincodeSBI[] = [];
  public filterCityList: ICitySBI[] = [];
  ...
  searchState!: IState[];
  searchPincode!: IPincodeSBI[];
  filteredCityList!: Observable<ICitySBI[]>;
  ...
  filteredautoPincodeList!: Observable<IPincodeSBI[]>;
  cityPincodes: IPincodeSBI[] = [];
  ...
  //   } else {
  //     this.CKYCData = true
  //     let g: { GenderCode: any; GenderDesc: any; } = null;
  //     let sts: IState = null;
  //     if (this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCGender) {

  //       g = this.genderList.find((g: IGender) => g!.GenderDesc[0] === this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCGender)

  //     }
  //     if (this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCCorAddState) {
  //       sts = this.stateList!.find((x: IState) => x!.StateCode == this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCCorAddState)
  //       this.getDistrictList(sts!.StateCode)
  ...
  //     this.ownerInfoForm.patchValue({
  //       ownerName: this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCFirstName,
  //       ownerLastName: this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCLastName,
  //       PropGenderTitle: this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCNamePrefix,
  //       gender: g !== null ? `${g.GenderCode},${g.GenderDesc}` : "",
  //       dob: new Date(this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCDOB),
  //       mobileNo: this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCMobileNumber,
  //       emailId: this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCEmailAdd,
  //     });


  //     this.addressForm.patchValue({
  //       pincode: this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCCorAddPin,
  //       postalAdd: this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCCorAdd1,
  //       state: sts !== null ? sts!.StateDesc : "",
  //       city: this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCCorAddCity,
  //       addArea: this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCCorAdd2,
  //       district: this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCCorAddDistrict
  //     });
  ...
  private setMortgageValidators() {
    ...
    const mort: any = this.bf.get("onMortgage")!.value
  ...
  private getDistrictList(StateCode?: any) {
    ...
    this._subscriptions.push(this._vehicleBuyPlanService.GetDistrict(StateCode).subscribe((result) => {
      ...
      if (this.ckycSessionData!.CorAddress.AddDistrict !== null) {
        const dts = this.districtList!.find((x: IDistrict) => x!.District_Name.toLowerCase() == this.ckycSessionData!.CorAddress!.AddDistrict.toLowerCase())
        this.getCityListSbi(dts!.State_code, dts!.District_code)
      } else {
  ...
  private getCityListSbi(StateCode: any, DistrictCode: any) {

    this._subscriptions.push(
      this._vehicleBuyPlanService.GetCity(StateCode, DistrictCode).subscribe((result) => {
        ...
        const cts: any = this.cityList!.find((x: ICitySBI) => x!.City_Name.toLowerCase() == this.ckycSessionData!.CorAddress.AddCity.toLowerCase())
        this.getPincodeListSbi(cts.State_code, cts.District_Code, cts.City_code)
  ...
  private getPincodeListSbi(StateCode: any, DistrictCode: any, CityCode: any) {
    let cts: any = this.addressForm.get("city")!.value;
    let city: ICitySBI | any = this.cityList.find(
    ...
    this._subscriptions.push(
      this._vehicleBuyPlanService.GetPincodeSBI(StateCode, DistrictCode, CityCode).subscribe((result) => {
        this.showLoader = false;
        if (result.successcode == "0") {
          this.errorLogDetails.UserCode = this.posMobileNo;
          this.errorLogDetails.ApplicationNo = this.postData.ApplicationNo;
      ...
      this._vehicleBuyPlanService.getPraposalMaster(this.PlanData.CompanyCode).subscribe(
        (result) => {
          ...
          if (result.successcode == "1") {
            ...
            if (this.ckycSessionData!.CKYCStatus !== null) {
  ...
  preFillProposalForm(dataToBePatched: any) {
    ...
    this.addressForm.patchValue({
      ...
      state: dataToBePatched.SBI_State_Name,
      city: dataToBePatched.SBI_City_Name,
      addArea: dataToBePatched.Area,
      district: dataToBePatched.SBI_District_Name
    });
  ...
  // private _filterCity(value: string): ICity[]  | ICitySBI[] {
  //   const filterBy = value ? value.toLowerCase() : value;
  ...
  private _filterPincode(value: string): IPincodeSBI[] {
  ...
  async onChangeState() {
    ...
    this._subscriptions.push(this._vehicleBuyPlanService.GetDistrict(stateCode).subscribe((result) => {
      ...
      this.districtList = result.data
      // const dts=this.districtList!.find((x:IDistrict)=>x!.District_Name==this.ckycSessionData!.A99ResponseData!.CKYCDownloadResult!.CKYCCorAddDistrict)
      // this.getCityListSbi(dts!.State_code,dts!.District_code)
      this.setDistrictValidator()
      // this.onChangeDistrictInput()
      console.log(this.districtList);
      this.filterDistrictList = this.districtList
    ...
    // this.onChangeDistrictInput(stateCode)
  }

  onChangeDistrictInput(stateCode?: any) {
  ...
  async onChangeDistrict() {
    ...
    let dts = this.addressForm.get("district")!.value;
  ...
  onChangeCityInput() {
    if (
      this.cityList.find(
        (City) => City.City_Name === this.addressForm.value.city
      )
    ) {
  ...
  public onSubmitPastPolicy(): any {
    ...
    rv.Area = add.addArea;
    // //
    let stateData: IState | any = this.stateList.find((x) => x.StateDesc == add.state);
    let districtData: IDistrict | any = this.districtList.find((x) => x.District_Name == add.district)
    let cityData: ICitySBI | any = this.cityList.find((x) => x.City_Name == add.city);
    ...
    if (add.district) {
      rv.SBI_District_Code = districtData.District_code;
      rv.SBI_District_Name = districtData.District_Name;
    }
    if (add.city) {
      rv.SBI_City_Code = cityData.City_code;
      rv.SBI_City_Name = cityData.City_Name;
      rv.CityDesc = cityData.City_Name
    }
    // //
    // if (add.state) {
    ...
    if (this.premiumDataPlans.istppd) {
      let TPPD = this.sessionData.premiumDataPlans.CoverageList.find(
        (x: any) => x.CoverageCode == "Third Party Property Damage"
      );
              ...
              .pipe(
                ...
                mergeMap((apiurlData) => {
                  ...
                  const filterdata: any = {
                    ...
                    IDVValue: 0,//this.PlanData.IDVVAlue,
  ...
  saveNewSessionData(item: IPlansMotor | any) {
    ...
    let idvBasic: any = item.CoverageList.find(
      (x: any) => x.CoverageCode === "IDV Basic"
    );
    let NCB: any = item.CoverageList.find(
      (x: any) => x.CoverageCode === "No Claim Bonus"
    );
    let TPPD: any = item.CoverageList.find((x: any) => x.CoverageCode === "TPPD");
    let PAOwner: any = item.CoverageList.find((x: any) => x.CoverageID == 16);
    let DepWaiver: any = item.CoverageList.find((x: any) => x.CoverageID == 1);


    let data: any = { ...this.premiumDataPlans };
...
export const thirdPartyDateValidator = (
  ...
  // const t = new Date();
  // console.log(e.getTime(), dateDiff.getTime(), tpExpiryDate, e.getTime() === dateDiff.getTime());


  if (dateDiff.getTime() != e.getTime())
    return { thirdPartyEndDateError: true };
...
function getAge(tpStart: any, tpEnd?: any) {
  ...
  let fiveYearDate = new Date(start.getFullYear() + 5, start.getMonth(), start.getDate() - 1)
...
//   };
// }
export function RequireMatchCity(cityList: ICitySBI[]) {
  return (control: AbstractControl) => {
...
export function RequireMatchPincode(pincode: IPincodeSBI[]) {
  return (control: AbstractControl) => {
