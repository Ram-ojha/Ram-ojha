...
import { MASKS, NCBList, PATTERN, Salutation, yesNoList } from 'src/app/models/common';
import { convertToDateFormat, convertToMydate, decrypt, encryption, setMaxDate, setMaxRegDate, setMinDate, setMinDateForNomineee } from 'src/app/models/common-functions';
import { ApplicationVehicleData, ICKYCBajaj, ICKYCDocListBajaj, ICKYCUniversal, ICity, IInsuranceCompany, IMortgageBank, IPincode, IRegistrationYear, IState, errorLog, ICKYCDocListUniversal } from 'src/app/models/common.Model';
import { IFamilyMembers, IGender, IGenderForCKYC, IHealthInsPlanUrlsResponse, IPlansMotor, Relation } from 'src/app/models/health-insu.Model';
import { EUniversalDocListEnum, EYesNo } from 'src/app/models/insurance.enum';
...
@Component({
  selector: 'app-buyplan-universal-shampoo',
  templateUrl: './buyplan-universal-shampoo.component.html',
  styleUrls: ['./buyplan-universal-shampoo.component.css', "../../buyplan/buyplan.component.css", "../../../bike-insurance/bike-plans/bike-plans.component.css"]
})
export class BuyplanUniversalShampooComponent implements OnChanges, OnInit, OnDestroy {
  ...
  get ckycSessionData() {
    return JSON.parse(sessionStorage.getItem("CKYCData")!)
  }
  posMobileNo: any;
  userName: any;
  ...
  fectchDataFromCKYCSession() {
    ...
    if (this.ckycSessionData === null) {
      ...
      if (this.ckycSessionData.CorAddress.AddState) {
        sts = this.stateList!.find((x: IState) => x!.StateShortDesc == this.ckycSessionData!.CorAddress.AddState)
      }
      ...
      this.ownerInfoForm.patchValue({
        ownerName: `${this.ckycSessionData!.FirstName} ${this.ckycSessionData!.MiddleName}`,
        ownerLastName: this.ckycSessionData!.LastName,
        PropGenderTitle: this.ckycSessionData!.Title,
      ...
      this._vehicleBuyPlanService.getPraposalMaster(this.PlanData.CompanyCode).subscribe(
        (result) => {
          ...
          if (result.successcode == "1") {
            ...
            if (this.premiumDataPlans.CompanyCode == '2' || this.premiumDataPlans.CompanyCode == '11' || this.premiumDataPlans.CompanyCode == '6') {
  ...
  public onSubmitPastPolicy(): any {
    ...
    rv.CoverageList = [];
    // rv.CKYCNumber = this.kycData ? this.kycData.ckycNumber : null;
  ...
  saveNewSessionData(item: IPlansMotor | any) {
    const premiumWithoutGst: any = this.addAddonsValue(item);
    const gstAmount: any = premiumWithoutGst * 0.18;
    ...
    let TPPD: any = item.CoverageList.find((x: any) => x.CoverageID == 28);
  ...
  public onSubmitVehicleInfo(): any {
    ...
    if (this.isNewPolicy || (this.isPrvPolicyRemember == 0 && this.PlanData.CompanyCode == '4')) {
      this.isCompleted_pp = true;
      this.onSubmitPastPolicy();
    } else {
...
export const pastpolicyExDateValidationForTATA = (
  ...
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    if (monthDiff <= 9) {
      return { postPolicyDateError: true };
    } else {
      return null
    }
  };
...
export const regDateValidationForAllOd = (
  ...
  if (regDate >= dateToBeCheckedAgainst) {
    ...
    if (monthDiff > 60) { return { OdError: true }; }
    else { return null }
