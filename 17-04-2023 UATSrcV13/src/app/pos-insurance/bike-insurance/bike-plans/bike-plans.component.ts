...
import { ApplicationVehicleData, errorLog, IList, MinMaxIDVInterface } from "src/app/models/common.Model";
import {
  calculatePercent,
  decrypt,
  encrypt,
...
export class BikePlansComponent implements OnInit, OnDestroy {
          ...
          if (result.successcode == "1" && result.data != null) {
            ...
            if (
              ...
              if (this.plan.SubCateCode == 1) {
                // Renew
                if (result.data.CompanyCode === '15') this.premiumCalculationForOriental(result, idvBasic, ncb)
                else {
  ...
  premiumCalculationForOriental(result: ApiResponse, idvBasic: { ODPremium: any; TPPremium: any; }, ncb: number | undefined) {
    ...
    let odPlusDiscount = Math.round((Number(idvBasic.ODPremium) + Number(result.data.DiscountAmt.replace(/--/g, "-"))));
    ...
    if (toCheckAboveHundred < 100) {
      const remainAmt = Math.round(100 - toCheckAboveHundred);
    ...
    if (this.plan.isComprehensive)
      if (ncb == undefined) {
        ...
        result.data.Premium = (toCheckAboveHundred < 100) ? Math.round(
  ...
  onBuyPlanClick1(item: IPlansMotor) {
    ...
    if (this.plan.SubCateCode == 1) {
      // Renew Case
      if (this.plan.isOdOnly == false && (item.CompanyCode === '11' || item.CompanyCode == '7') && (this.PaCoverReason === null || this.PaCoverReason === undefined)) {
        this.onBuyPlanClickPACoverQustion()
      }
      ...
      // New case
      // this.onBuyPlanClick(item);
      if ((item.CompanyCode === '11' || item.CompanyCode === '7') && this.plan.paCover !== true && (this.PaCoverReason === null || this.PaCoverReason === undefined)) {
        this.onBuyPlanClickPACoverQustion()
      } else {
        this.onBuyPlanClick(item);
      }
  ...
  onBuyPlanClick(item: IPlansMotor) {
    ...
    // }
    // else 
    this.premiumDataPlans = this.popupData(item);
    const sesDataSet = {
      rto: this.plan.rto,
  ...
  popupData(item: IPlansMotor | any) {
    ...
    if ((item.ProductName == 'The Oriental Insurance Company Ltd.') && !this.plan.isTp && this.plan.SubCateCode === 1) {
      if (this.plan.zeroDp) {
        ...
        NCB.ODPremium = (Math.round((-calNcb) * (this.plan.currentNcb / 100))).toString();
        // console.log(NCB);
    ...
    if (
      ...
      item["DiscountAmt"] = Math.round(Number(item["DiscountAmt"])).toString().replace(/--/g, "-");
    }
    ...
    if (ind === 2) {
      ...
      if (!this.plan.isOdOnly) {
        ...
        for (let i = 0; i < this.planResponse.length; i++) {
          ...
          this.filteredPlans[i].Premium = Math.round(+coverageList.ODPremium);
  ...
  showDefaultValue(ind?: number) {
    ...
    for (let i = 0; i < this.filteredPlans.length; i++) {
      ...
      if (ind === 0) {
        ...
        // isOdOnly
        this.filteredPlans[i].Premium = Math.round(Number(idvBasic.ODPremium));
      } else {
        // isTp
        // this.filteredPlans = [];
        // for (let i = 0; i < this.planResponse.length; i++) {
        ...
        //   );
        // }
        this.filteredPlans[i].Premium = Math.round(Number(idvBasic.TPPremium));
      }
    }
  }
  ...
  onChangeZeroDep() {
    ...
    for (let i = 0; i < this.filteredPlans.length; i++) {
      ...
      if (this.filteredPlans[i].CompanyCode === '15' && this.plan.SubCateCode == 1) this.popupData(this.filteredPlans[i]);
      else if (depWaiver && +depWaiver.ODPremium != 0) {
        this.filteredPlans[i].Premium = this.plan.zeroDp
          ? Math.round(this.filteredPlans[i].Premium) +
          Math.round(Number(depWaiver.ODPremium))
          : Math.round(this.filteredPlans[i].Premium) -
          Math.round(Number(depWaiver.ODPremium));
      }
    }
  ...
  premiumWithoutDiscount(item: { DiscountAmt: string; ProductName: string; CoverageList: any[]; Premium: string | number; }) {
    ...
    if (this.NCB && DiscountAmt) {
      if (!this.plan.isTp)
        actualAmount =
          +this.NCB != 0
            ? String(Math.round(+item.Premium + +this.NCB - DiscountAmt))
            : "";
    } else if (this.NCB) {
      if (!this.plan.isTp)
        actualAmount =
          +this.NCB != 0
            ? String(Math.round(+item.Premium + +this.NCB))
            : "";
  ...
  openResponseModal() {
    ...
    // console.log("hi----<>",this.responseResult)
