...
@Component({
  selector: 'app-buyplan-hdfc-ergo',
  templateUrl: './buyplan-hdfc-ergo.component.html',
  styleUrls: ['./buyplan-hdfc-ergo.component.css']
})
export class BuyplanHdfcErgoComponent implements OnInit, OnDestroy {
  ...
  filteredPincode!: Observable<IPincode[]> | any;
  ...
  filterState!: IState[];
  getRTO: any;
  filterPincode!: IPincode[];
  filterPin!: IPincode[];
  pincode!: IPincode[];
  searchCity!: ICity[];
  filteredPincodeListForMemeber!: Observable<IPincode[]>;
  searchPin!: IPincode[];
  ...
  public onSelectSalutation1(e: any) {

    console.log(this.insuredMembersForm, e)
    let salutation = e.controls["GenderTitle0"].get("PropGenderTitle");
    console.log(e)
    if (e.value == "Mr") {
  ...
  // async onChangeState() {
  //   
  //   const sts = this.communicationAddressForm.get("state").value; //st.name
  ...
  //   this.filteredCityList = this.communicationAddressForm.controls["city"].valueChanges.pipe(
  //     startWith(""),
      ...
      this._healthBuyPlanService
        ...
        .subscribe(
          (result) => {
            ...
            this.filterState = result.data.state;
            ...
            this.filteredCityListForMemebers =
              this.communicationAddressForm.controls["city"].valueChanges.pipe(
                startWith(""),
                map((city) => this._filterCityforInsurer(city))
              );
            this.filteredPincodeListForMemeber =
              this.communicationAddressForm.controls["AddPincode"].valueChanges.pipe(
                startWith(""),
            ...
            if (result.successcode == "1" && result.data) {
              ...
              this.filterStateList = result.data["state"];
              this.filterPincode = result.data["pincode"]
              // this.setPincode();
              // this.getArea();
              ...
              if (result.successcode == "0") {
                ...
                this.errorLogDetails.CompanyName = 'Hdfc-ergo'
  ...
  async onSelectState(formName: string): Promise<any> {
    if (formName === "proposer") {
      ...
      console.log("state code --=> ", stateCode, stateDesc)
      this.communicationAddressForm.patchValue({
        StateDesc: stateDesc,
        StateCode: stateCode,
      });
      ...
      this.filteredCityListForMemebers = this.communicationAddressForm.controls[
        ...
        startWith(""),
        map((city) => this._filterCityforInsurer(city, citiesForSelectedState))
      );
    }
  }
  ...
  onSelectCity(formName: any) {
    ...
    this.filteredPincode = this.proposerForm.controls[
      ...
      startWith(""),
      map((pin) => this._filterPincodeforproposer(pin, pincodesForSelectedCity))
    );
  ...
  async AddressononSelectCity(): Promise<any> {
    ...
    this.communicationAddressForm.controls["AddPincode"].reset();
    this.filteredPincode = this.statePincodes.filter(
      (pin) => pin.CityCode == city
    );
  ...
  async onChangeStateInput() {
    ...
    if (
      this.stateList.find(
        (state) => state.StateDesc === this.communicationAddressForm.value.state
      )
    )
      this.onChangeState();
  }
  ...
  async onChangeState() {
    ...
    console.log("--=>", sts, this.searchState)
    let stateCode = this.searchState[0].StateCode;

    this.filterCityList = stateCode
      ? this.cities.filter((city: ICity) => city.StateCode == stateCode)
      : [];
    console.log("--=>", this.filterCityList, stateCode)
  ...
  onChangeCityInput() {
    if (
      this.cities.find(
        (City) => City.CityDesc === this.communicationAddressForm.value.city
      )
    )
      this.onChangeCity();
  }
  ...
  async onChangeCity() {
    ...
    let cts = this.communicationAddressForm.get("city")!.value;
    this.showLoader = true;
    console.log(cts)
    this.showLoader = false;
    this.searchCity = cts
      ? this.cities.filter((st: ICity) => st.CityDesc == cts)
      : [];

    let cityCode = this.searchCity[0].CityCode;
    // this.pincode = this.pincodeList.filter((pin:IPincode)=> pin.CityCode == cityCode)
    // console.log(this.pincode)

    this._vehicleBuyPlanService.GetPincode(cts, this.searchState[0].StateDesc).subscribe((result) => {
      this.showLoader = false;
      if (result.successcode == "0") {
        this.errorLogDetails.UserCode = this.posMobileNo;
        this.errorLogDetails.ApplicationNo = this.ApplicationNo;
      ...
      this.filteredautoPincodeList = this.communicationAddressForm.controls[
        "AddPincode"
      ].valueChanges.pipe(
        startWith(""),
  ...
  async onChangePinCodeInput() {
    ...
    if (
      !this.pincodeList.find(
        (pincode) => pincode.Pincode === this.communicationAddressForm.value.pincode
      )
    ) {
  ...
  async onChangePincode() {
    ...
    const pin = this.communicationAddressForm.get("AddPincode")!.value;
    ...
    this.showLoader = false;
    this.searchPin = pin
      ? this.pincodeList.filter((st: IPincode) => st.Pincode == pin)
      : [];


    let pinCode = this.searchPin[0].Pincode;
    this.pincode = this.pincodeList.filter((pin: IPincode) => pin.Pincode == pinCode)
    console.log(this.pincode)
    this.setPincodeValidator();
    this.filteredPincode = this.communicationAddressForm.controls[
      "AddPincode"
    ...
    const found: any = this.cities.find(
      (City) => City.CityDesc === this.communicationAddressForm.value.city
    );
    console.log(found);
    const stateCodeForMember = found['StateCode'];
    const cityIdForMember = found['CityCode'];
    const pincodeForMember = this.communicationAddressForm.get('AddPincode')!.value
    this.areaForMembers = this.getAreaEndPoint(
      stateCodeForMember,
      cityIdForMember,
      pincodeForMember
  ...
  setPincodeValidator() {
    ...
    this.af.get("AddPincode")!.updateValueAndValidity();
  }
  ...
  private _filterCityforInsurer(value: string, cityList?: ICity[]): ICity[] {
    const filterBy = value ? value.toLowerCase() : value;
  ...
  private _filterPincodeforproposer(value: string, pincodeList?: IPincode[]): IPincode[] {
    const filterBy = value;

    return pincodeList!.filter(
      (option) => option.Pincode.toString().trim().indexOf(filterBy) === 0
    );
