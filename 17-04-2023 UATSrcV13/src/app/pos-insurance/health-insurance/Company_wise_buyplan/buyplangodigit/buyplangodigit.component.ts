...
@Component({
  templateUrl: "./buyplangodigit.component.html",
  styleUrls: ["./buyplangodigit.component.css"],
  encapsulation: ViewEncapsulation.None,
})
export class BuyplanGodigitComponent implements OnInit, OnDestroy {
      ...
      this._healthBuyPlanService.getHealthMasterData(id, idOdp, c_code).subscribe(
        (result) => {
          ...
          if (result.successcode == "1" && result.data) {
            ...
            this.nomineerelationList = this.nomineerelationList.filter(
    ...
    if (event.checked) {
      ...
      if (op == "none") {
        ...
        let QuestionValue = this.medicalHis.filter(
          (m) => m.parentQuestionID == node.QuestionID && m.subchild == 1
        );
      ...
      // if (existCnt.length == 1 ||(selectedMemberCode == 0 && op == "none" && parent.length >= 0)) {
      if (existCnt.length == 1 && op == "none") {
      ...
      let QuestionValue = this.medicalHis.filter(
        (m) =>
          m.FamilyMemberCode == selectedMemberCode &&
          m.RecordNo == RecordNo &&
          m.QuestionValue == "Yes" &&
          m.subchild == 1
      );
  ...
  async onSubmitReview(): Promise<any> {
    ...
    for (var keys in medicalHisValidation) {
      var ind = 0;
      let tvalue;
    ...
    if (this.memberDetails.length == 1 || this.memberDetails.length == 2) {
      ...
      if (this.memberDetails.length == List.length) {
        this.praposalrelationList = this.praposalrelationLists.filter(
          (m: any) => m.FamilyMemberCode == 5 || m.FamilyMemberCode == 6
        );
      } else {
        this.praposalrelationList = this.praposalrelationLists.filter(
          (m: any) => m.FamilyMemberCode != 1
        );
      }
    } else {
      this.praposalrelationList = this.praposalrelationLists.filter(
  ...
  CheckboxQuestionAdd(
    ...
    console.log(child1, md1);
    if (event.checked) {
      this.medicalHis.push({
        QuestionID: child1.QuestionID,
        FamilyMemberCode: md1.FamilyMemberCode,
        QuestionValue: "Yes",
        RecordNo: md1.RecordNo,
      ...
      let index = this.medicalHis.findIndex(
        (m) =>
          m.QuestionID == child1.QuestionID &&
          m.RecordNo == md1.RecordNo &&
          m.subchild == 1
      );
      if (index !== -1) {
        this.medicalHis.splice(index, 1);
  ...
  noneForAll(event: any) {
    for (const question of this.companyQueTree) {
      ...
      else frmGroup.controls[`none${question.QuestionID}`].setValue("");
    }
  }
  ngOnDestroy() {
