...
@Component({
  ...
  styleUrls: ["./buyplan-sbi.component.css"],
})
export class BuyplanSbiComponent implements OnInit, OnDestroy {
          ...
          (result) => {
            ...
            this.filteredCityListForMemebers =
              this.communicationAddressForm.controls["city"].valueChanges.pipe(
                startWith(""),
                map((city) => this._filterCity(city, this.cities))
              );
  ...
  getArea() {
    ...
    const stateCodeForMember: string | undefined = this.stateList.find(
      (state: any) =>
        state.StateDesc.toUpperCase() ===
        this.memberDetails[0].StateDesc.toUpperCase()
    )!.StateCode;

    const cityIdForMember: string | undefined = this.cities.find(
      (city: any) =>
  ...
  private _filterCity(value: string, cityList: ICity[]): ICity[] {
    ...
    return cityList.filter(
      (row: any) => row.CityDesc.toLowerCase().indexOf(filterBy) === 0
    );
  }
  private _filterArea(value: string, areaData: any): ICity[] {
    ...
    return areaData["data"].filter(
      (row: any) => row["AreaDesc"].toLowerCase().indexOf(filterBy) === 0
    );
  }
