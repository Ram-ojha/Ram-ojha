...
import { ICKYCBajaj, ICKYCDocList, ICKYCDocListUniversal, ICKYCFutureGen, ICKYCGoDigit, ICKYCNewIndia, ICKYCReliance, ICkycResponse, ICKYCSBI, ICKYCShreeRam, ICKYCTataAig, ICKYCUniversal, IInsuranceType } from 'src/app/models/common.Model';
...
export class CkycDetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.PlanData.CompanyCode === "9" || this.PlanData.CompanyCode === "4" || this.PlanData.CompanyCode === "12") {
  ...
  onSelectCustomerType(event: MatSelectChange) {
    ...
    } else if (this.PlanData.CompanyCode == "2") {
      if (event.value === 'I') {
        this.CkycDocListFilter = this.CkycDocList.filter((x: ICKYCDocList) => { return (x.ApplicableFor === 'Ind' || x.ApplicableFor === 'Both') })
  ...
  public onSubmitCKYCInfoForUnitedIndia(): any {
    ...
    let cData = this.ckycDetailForm.value;
    let dob = new Date(cData.DOB)
    let formatDOB = formatDate(dob, 'dd/MM/yyyy', 'en-US'); //dd-MMM-yyyy
    console.log(this.ckycDetailForm)
    const ckycData: any = {
      ApplicationNo: this.postData.ApplicationNo,
      CompanyCode: this.PlanData.CompanyCode,
    ...
    this._subscriptions.push(
      this._vehicleBuyPlanService.getUnitedIndiaValidateCKYC(ckycData).subscribe((res: any) => {
        debugger
        console.log(res);
  ...
  onSubmitCKYCInfoShreeRam(): any {
    ...
    console.log(cData, this.CKYCHideShowShreeRam);
  ...
  onSelectStep(event: any): any {
    ...
    if (this.PlanData.CompanyCode === "9") {
      ...
      console.log(event);
      const formControl = ["doctypePOI", "docNoPOI", "doctypePOA", "docNoPOA", "docDataPOA", "docDataPOI"];
      this.ckycDetailForm.markAsUntouched()
      if (event === "fetch") {
  ...
  getCkycDocList() {
    ...
    this._subscriptions.push(
      this.posHomeService.getCKYCDocList(CompanyCode).subscribe((result: any) => {
        ...
        if (CompanyCode == '7' || CompanyCode == '2' || CompanyCode == '11' || CompanyCode == '4') {
          this.CkycDocListFilter = this.CkycDocList.filter((x: ICKYCDocList) => { return (x.ApplicableFor === "Ind" || x.ApplicableFor === "Both") })
