...
export class BikePolicyComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.getBikePolicyData();
    this.getBikePendingPaymentData();
    this.fieldListener();
  }
  ...
  public getBikePolicyData() {
    this.loading = true;
    this._subscriptions.push(
      this._policyBoardService.policySearch(Indication.BIKE).subscribe(
        (response) => {
          this.loading = false;
  ...
  getBikePendingPaymentData() {
    this.loading = true;
    this._subscriptions.push(
      this._policyBoardService
        .policySearch(Indication.BIKE_PAYMENT_PENDING)
        .subscribe(
          (response) => {
            this.loading = false;
            console.log(response);
